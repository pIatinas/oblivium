...
const MembersPage = () => {
  ...
  const [memberStats, setMemberStats] = useState<{ [key: string]: MemberStats }>({});
    ...
    try {
      ...
      if (profiles) {
        ...
        for (const profile of profiles) {
          // Count knights
          const { count: knightsCount } = await supabase
            .from('user_knights')
  ...
  const handleMemberClick = (member: Profile) => {
    ...
    navigate(`/members/${slug}`, { state: { userId: member.user_id } });
  };
  ...
  return (
    <>
      <SEOHead 
        title="Membros - Oblivium"
        description={`Conheça os ${members.length} membros da comunidade Oblivium`}
      />
      <div className="min-h-screen bg-gradient-nebula">
        <Header />
          ...
          {filteredMembers.length > 0 ? (
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filteredMembers.map((member) => {
                const stats = memberStats[member.user_id] || { knights: 0, battles: 0, comments: 0 };
                return (
                  <Card 
                    key={member.id}
